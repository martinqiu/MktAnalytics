...
if /I "%RSCRIPT%"=="Rscript" (
  where %RSCRIPT% >nul 2>nul
  if %ERRORLEVEL% neq 0 (
    echo Rscript was not found. Install R and ensure Rscript is on PATH, or update launch_dashboard.bat with your local R path.
    pause
    exit /b 1
  )
) else (
...
"%RSCRIPT%" "planning_dashboard\run_app.R"
